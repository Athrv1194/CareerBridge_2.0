package com.careerbridge.prs.exception;
...
public class GlobalExceptionHandler {
     ...
     * The parse detail is withheld: it echoes request content back to the caller.
     *
     * Every endpoint here is a GET, so nothing triggers this today. Kept anyway: the other eight
     * services all carry it, and the day a POST is added this is the handler nobody remembers to
     * write.
     */
    ...
    /**
     * A non-numeric X-User-Id or X-User-Org-Id, or an unconvertible {studentId} path variable. Same
     * trap as the handler above: MethodArgumentTypeMismatchException extends TypeMismatchException,
     * which does NOT implement ErrorResponse, so it would otherwise fall through to a 500 for what
     * is purely a malformed client request. Only the parameter name is echoed, never the offending
     * value.
     *
     * This one is live from day one here -- GET /api/prs/{studentId} takes a Long path variable.
     */
