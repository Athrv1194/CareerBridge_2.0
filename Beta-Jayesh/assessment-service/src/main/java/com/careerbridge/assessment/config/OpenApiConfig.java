    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        ...
                        .version("1.0.0")
                        .description("CareerBridge Assessment Service API"))
                // Overrides springdoc's request-derived default, which -- when this spec is
                // fetched by the gateway's aggregated Swagger UI via its /api-docs/assessment
                // rewrite route -- would otherwise report this container's internal hostname
                // (http://assessment-service:8083), unreachable from a browser. Every request in
