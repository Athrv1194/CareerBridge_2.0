    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        ...
                        .version("1.0.0")
                        .description("CareerBridge Student Service API"))
                // Overrides springdoc's request-derived default, which -- when this spec is
                // fetched by the gateway's aggregated Swagger UI via its /api-docs/student rewrite
                // route -- would otherwise report this container's internal hostname
                // (http://student-service:8082), unreachable from a browser. Every request in this
