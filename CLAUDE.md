...
|---|---|---|---|---|
| API Gateway | `api-gateway/` | 8080 | — | Spring Cloud Gateway (servlet/webmvc flavor, Spring Cloud 2025.1.2 / SCG 5); validates JWTs and injects `X-User-Id`/`X-User-Role`/`X-User-Org-Id`, the only component in the system that does; routes `/api/{auth,student,assessment,recommendation,notification,organization,roadmap,prs,recruiter}/**` to the other services by `${..._SERVICE_URI}` placeholders in `application.yml`, defaulting to `localhost:<port>` |
...
| PRS Service | `prs-service/` | 8089 | PostgreSQL `careerbridge_prs` | **Third P1 service.** Placement Readiness Score: a weighted composite of assessment (40%), roadmap (30%) and profile (20%) completion. Consumes three events on three queues, publishes `prs.updated`. One of two services that make a synchronous HTTP call to another service (student-service, for the profile input) |
| Recruiter Service | `recruiter-service/` | 8090 | PostgreSQL `careerbridge_recruiter` | **Fourth P1 service.** Recruiter portal: company profiles, job postings, applications, candidate search, interview scheduling. Publishes `application.submitted` and `application.status.updated`, deliberately unconsumed. The other service with synchronous HTTP calls — two of them, to student-service and prs-service, neither through the gateway |
...
All 10 services use `springdoc-openapi-starter-webmvc-ui:2.8.9` — **webmvc, not webflux, for every service including api-gateway**, since none of them are reactive (see api-gateway's own section below; this has already been the source of two prior incidents, do not revisit it).
...
The whole stack is containerized — `docker compose --env-file .env up -d --build` brings up 11 containers (postgres, rabbitmq, 8 backend services, api-gateway). Only the gateway (8080), postgres (host 5434) and rabbitmq (5672/15672) publish host ports. The list below is what each service expects when run directly on the host with `mvnw spring-boot:run` instead:
...
- **student-service — complete** (profile CRUD, education/skills/projects/certificates, profile completion scoring, RabbitMQ `student.registered` consumer that auto-creates profiles, 28 unit tests) — branch `feature/student`. Extended for recruiter-service (see below): `GET /api/student/profiles/public` and `StudentProfile.role`, both on `feature/recruiter-service`.
...
- **prs-service — complete** (third P1; Placement Readiness Score, a 40/30/20 weighted composite with 10% reserved; consumes `student.registered`, `recommendation.generated` and `roadmap.updated` on three queues; one of two services with a synchronous cross-service HTTP call; tenant-scoped leaderboard; publishes `prs.updated`; 24 tests) — branch `feature/prs-service`. Verified live end to end, including the fail-soft client with student-service deliberately stopped.
...
- **The weights sum to 0.90, not 1.00, so `totalScore` tops out at 90.** The missing 10% is reserved for a future resume builder. `PrsBreakdown` reports it as `reservedWeight: 10, reservedContribution: 0.0` so the gap explains itself. **Do not normalise to 100** — every stored score would shift the day the resume builder ships, and the breakdown's four contributions would stop summing to the total. Grades: `>=80 A`, `>=60 B`, `>=40 C`, `>=20 D`, else `F`, lower bound inclusive.
- **The three raw inputs are stored unweighted**; `totalScore` is always recomputed from all three, never incremented. Re-tuning a weight therefore does not invalidate stored data.
...
- **All 9 databases must exist before any service starts** (`CREATE DATABASE careerbridge_auth;` `_student;` `_assessment;` `_recommendation;` `_notification;` `_organization;` `_roadmap;` `_prs;` `_recruiter;` — PostgreSQL has no `createDatabaseIfNotExist` JDBC flag, unlike MySQL). Add this to team setup notes alongside the `application-local.yml` step. Under Docker `docker/init.sql` handles it, but only on a fresh `postgres-data` volume — an existing one needs each new database created by hand.
