...
- **`findByStudentIdAndStatusOrderByStartedAtDesc` returns a `List`, not an `Optional`, and that is load-bearing.** A student who takes a second assessment gets a second recommendation and therefore a second roadmap, while the first stays `IN_PROGRESS` — so the query legitimately matches several rows and an `Optional` finder would throw `IncorrectResultSizeDataAccessException` on exactly the second assessment. `getMyRoadmap` takes the newest. Same shape recommendation-service settled on for `findByUserIdAndIsActiveTrue`; the original task spec asked for an `Optional` here and it was wrong.
