...
|---|---|---|---|---|
| API Gateway | `api-gateway/` | 8080 | — | Spring Cloud Gateway (servlet/webmvc flavor, Spring Cloud 2025.1.2 / SCG 5); validates JWTs and injects `X-User-Id`/`X-User-Role`/`X-User-Org-Id`, the only component in the system that does; routes `/api/{auth,student,assessment,recommendation,notification,organization,roadmap,prs}/**` to the other services by `${..._SERVICE_URI}` placeholders in `application.yml`, defaulting to `localhost:<port>` |
...
All 9 services use `springdoc-openapi-starter-webmvc-ui:2.8.9` — **webmvc, not webflux, for every service including api-gateway**, since none of them are reactive (see api-gateway's own section below; this has already been the source of two prior incidents, do not revisit it).
...
- **prs-service — complete** (third P1; Placement Readiness Score, a 40/30/20 weighted composite with 10% reserved; consumes `student.registered`, `recommendation.generated` and `roadmap.updated` on three queues; the only synchronous cross-service HTTP call in the system; tenant-scoped leaderboard; publishes `prs.updated`; 24 tests) — branch `feature/prs-service`. Verified live end to end, including the fail-soft client with student-service deliberately stopped.

All 6 P0 services plus 3 P1 services implemented, docker-compose in place, and the full P0 event chain (auth → student → assessment → recommendation → notification) verified live against real infrastructure on 2026-07-29.
