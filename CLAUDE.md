...
|---|---|---|---|---|
| API Gateway | `api-gateway/` | 8080 | — | Spring Cloud Gateway (servlet/webmvc flavor, Spring Cloud 2025.1.2 / SCG 5); validates JWTs and injects `X-User-Id`/`X-User-Role`/`X-User-Org-Id`, the only component in the system that does; routes `/api/{auth,student,assessment,recommendation,notification,organization,roadmap}/**` to the other services by `${..._SERVICE_URI}` placeholders in `application.yml`, defaulting to `localhost:<port>` |
...
All 8 services use `springdoc-openapi-starter-webmvc-ui:2.8.9` — **webmvc, not webflux, for every service including api-gateway**, since none of them are reactive (see api-gateway's own section below; this has already been the source of two prior incidents, do not revisit it).
...
The whole stack is containerized — `docker compose --env-file .env up -d --build` brings up 10 containers (postgres, rabbitmq, 7 backend services, api-gateway). Only the gateway (8080), postgres (host 5434) and rabbitmq (5672/15672) publish host ports. The list below is what each service expects when run directly on the host with `mvnw spring-boot:run` instead:
- PostgreSQL on `localhost:5432`; the 7 databases (`careerbridge_auth`, `_student`, `_assessment`, `_recommendation`, `_notification`, `_organization`, `_roadmap`) must be created manually first — `CREATE DATABASE ...;` — PostgreSQL has no `createDatabaseIfNotExist` JDBC flag the way MySQL did. Under Docker `docker/init.sql` creates them, but **only on a fresh volume**
...
| `RecommendationGeneratedEvent` | `recommendation.generated` | recommendation-service | notification-service (email + in-app notification), roadmap-service (generates the milestone roadmap) | **two consumers, each on its own queue.** notification-service's leg is verified live; roadmap-service's is not yet |
...
| `RoadmapUpdatedEvent` | `roadmap.updated` | roadmap-service | *(none yet)* | **published, deliberately unconsumed.** Intended for a future placement-service, where `completionPercentage` carries 30% of the Placement Readiness Score. No queue declared, same rule as `organization.created` |
...
All 6 P0 services plus 2 P1 services implemented, docker-compose in place, and the full P0 event chain (auth → student → assessment → recommendation → notification) verified live against real infrastructure on 2026-07-29.
...
- Since this gateway is the only thing validating a JWT, **ports 8081–8088 must not be publicly reachable** at deploy time — the same constraint already noted for student-service's port 8082, now true of all seven backend services. `docker-compose.yml` already enforces this by publishing no host port for any of them.
